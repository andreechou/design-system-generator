...
handleEvent('createSystem', (state) => {
	async function main() {
		...
		frame.paddingLeft = Number(state.paddingHor);
		frame.paddingRight = Number(state.paddingHor);
		frame.paddingTop = Number(state.paddingVer);
		frame.paddingBottom = Number(state.paddingVer);
		frame.cornerRadius = Number(state.borderRadius);
